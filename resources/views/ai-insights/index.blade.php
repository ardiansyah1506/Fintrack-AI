<x-app-layout title="AI Insights - AI Center">
    <x-slot name="header">AI Insights</x-slot>
    <div x-data="{ showModal: false, editMode: false, currentId: null, form: {'title': '', 'period': '', 'content': '', 'generated_at': '', 'type': ''} }" class="space-y-6 text-slate-800 font-sans relative">
        <div class="flex justify-between items-center bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
            <div>
                <h2 class="font-bold text-xl text-slate-800">Manajemen AI Insights</h2>
            </div>
            <button @click="showModal = true; editMode = false; form = {'title': '', 'period': '', 'content': '', 'generated_at': '', 'type': ''}" class="bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2.5 rounded-2xl shadow-sm text-sm font-semibold transition">
        ...
        <div class='mt-4'>{{ $ai_insights->appends(['search' => request('search')])->links() }}</div>
        ...
        <div x-show="showModal" style="display:none;" class="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div class="bg-white rounded-3xl p-6 w-full max-w-lg z-10 shadow-2xl relative" x-transition>
                ...
                <form :action="editMode ? '{{ url('ai-insights') }}/' + currentId : '{{ url('ai-insights') }}'" method="POST" class="space-y-4 max-h-[70vh] overflow-y-auto">
                    @csrf
                    ...
                    <div>
                        ...
                        <input type='text' name='period' x-model='form.period' class='w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500' required>
                    </div>
                    <div>
                        <label class='block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2'>content</label>
                        <input type='text' name='content' x-model='form.content' class='w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500' required>
                    </div>
                    <div>
