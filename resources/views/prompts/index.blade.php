<x-app-layout title="Prompts - AI Center">
    <x-slot name="header">Prompts</x-slot>
    <div x-data="{ showModal: false, editMode: false }" class="space-y-6 text-slate-800 font-sans relative">
        <div class="flex justify-between items-center bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
            <div>
                <h2 class="font-bold text-xl text-slate-800">AI Prompts</h2>
            </div>
        ...
        <div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
            <table class="w-full text-left text-sm whitespace-nowrap">
                <thead class="bg-slate-50 text-slate-500 text-xs font-semibold uppercase tracking-wider"><tr><th class='px-6 py-4'>name</th>
<th class='px-6 py-4'>description</th>
<th class='px-6 py-4'>content</th>
<th class='px-6 py-4'>is_active</th>
        ...
        <div x-show='showModal' style='display:none;' class='fixed inset-0 z-50 flex items-center justify-center'>
            ...
            <div class='bg-white rounded-3xl p-6 w-full max-w-lg z-10 shadow-2xl relative' x-transition>
                <form action="{{ route('prompts.store') }}" method="POST" class="space-y-4">
                    ...
                    <input type='text' name='name' class='w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none' required>
                    </div>
