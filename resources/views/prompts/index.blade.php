<x-app-layout title="Prompt Manager - AI Center">
    <x-slot name="header">Prompt Manager</x-slot>
    <div x-data="{ showModal: false, editMode: false, currentId: null, form: {'name': '', 'prompt': '', 'active': '', 'version': ''} }" class="space-y-6 text-slate-800 font-sans relative">
        <div class="flex justify-between items-center bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
            <div>
                <h2 class="font-bold text-xl text-slate-800">Manajemen Prompt Manager</h2>
            </div>
            <button @click="showModal = true; editMode = false; form = {'name': '', 'prompt': '', 'active': '', 'version': ''}" class="bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2.5 rounded-2xl shadow-sm text-sm font-semibold transition">
        ...
        <div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
            <table class="w-full text-left text-sm whitespace-nowrap">
                <thead class="bg-slate-50 text-slate-500 text-xs font-semibold uppercase tracking-wider">
                    <tr>
                        <th class='px-6 py-4'>name</th><th class='px-6 py-4'>prompt</th><th class='px-6 py-4'>active</th><th class='px-6 py-4'>version</th>
                ...
                <tbody class="divide-y divide-slate-100 text-slate-700">
                    ...
                    <tr class="hover:bg-slate-50">
                        <td class='px-6 py-4 truncate max-w-xs'>{{ $item->name }}</td><td class='px-6 py-4 truncate max-w-xs'>{{ $item->prompt }}</td><td class='px-6 py-4 truncate max-w-xs'>{{ $item->active }}</td><td class='px-6 py-4 truncate max-w-xs'>{{ $item->version }}</td>
                        <td class="px-6 py-4 text-right">
                            <button @click="showModal = true; editMode = true; currentId = {{ $item->id }}; form = {'name': '{{ addslashes($item->name ?? "") }}', 'prompt': '{{ addslashes($item->prompt ?? "") }}', 'active': '{{ addslashes($item->active ?? "") }}', 'version': '{{ addslashes($item->version ?? "") }}'}" class="text-indigo-600 hover:text-indigo-800 mx-2 text-xs"><i class="fa-solid fa-pen"></i></button>
                            <form action="{{ url('prompts', $item->id) }}" method="POST" class="inline">
                                @csrf @method('DELETE')
        ...
        <div class='mt-4'>{{ $prompts->appends(['search' => request('search')])->links() }}</div>
        ...
        <div x-show="showModal" style="display:none;" class="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div class="bg-white rounded-3xl p-6 w-full max-w-lg z-10 shadow-2xl relative" x-transition>
                ...
                <form :action="editMode ? '{{ url('prompts') }}/' + currentId : '{{ url('prompts') }}'" method="POST" class="space-y-4 max-h-[70vh] overflow-y-auto">
                    @csrf
                    <template x-if="editMode"><input type="hidden" name="_method" value="PUT"></template>
                    
                    <div>
                        <label class='block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2'>name</label>
                        <input type='text' name='name' x-model='form.name' class='w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500' required>
                    </div>
                    <div>
                        <label class='block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2'>prompt</label>
                        <input type='text' name='prompt' x-model='form.prompt' class='w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500' required>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input type='text' name='version' x-model='form.version' class='w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500' required>
                    </div>
