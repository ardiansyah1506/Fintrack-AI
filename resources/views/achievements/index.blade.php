<x-app-layout title="AI Achievements - AI Center">
    <x-slot name="header">AI Achievements</x-slot>
    <div x-data="{ showModal: false, editMode: false, currentId: null, form: {'title': '', 'description': '', 'reward': '', 'achieved_at': ''} }" class="space-y-6 text-slate-800 font-sans relative">
        <div class="flex justify-between items-center bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
            <div>
                <h2 class="font-bold text-xl text-slate-800">Manajemen AI Achievements</h2>
            </div>
            <button @click="showModal = true; editMode = false; form = {'title': '', 'description': '', 'reward': '', 'achieved_at': ''}" class="bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2.5 rounded-2xl shadow-sm text-sm font-semibold transition">
        ...
        <div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
            <table class="w-full text-left text-sm whitespace-nowrap">
                <thead class="bg-slate-50 text-slate-500 text-xs font-semibold uppercase tracking-wider">
                    <tr>
                        <th class='px-6 py-4'>title</th><th class='px-6 py-4'>description</th><th class='px-6 py-4'>reward</th><th class='px-6 py-4'>achieved_at</th>
                ...
                <tbody class="divide-y divide-slate-100 text-slate-700">
                    ...
                    <tr class="hover:bg-slate-50">
                        <td class='px-6 py-4 truncate max-w-xs'>{{ $item->title }}</td><td class='px-6 py-4 truncate max-w-xs'>{{ $item->description }}</td><td class='px-6 py-4 truncate max-w-xs'>{{ $item->reward }}</td><td class='px-6 py-4 truncate max-w-xs'>{{ $item->achieved_at }}</td>
                        <td class="px-6 py-4 text-right">
                            <button @click="showModal = true; editMode = true; currentId = {{ $item->id }}; form = {'title': '{{ addslashes($item->title ?? "") }}', 'description': '{{ addslashes($item->description ?? "") }}', 'reward': '{{ addslashes($item->reward ?? "") }}', 'achieved_at': '{{ addslashes($item->achieved_at ?? "") }}'}" class="text-indigo-600 hover:text-indigo-800 mx-2 text-xs"><i class="fa-solid fa-pen"></i></button>
                            <form action="{{ url('achievements', $item->id) }}" method="POST" class="inline">
                                @csrf @method('DELETE')
        ...
        <div class='mt-4'>{{ $achievements->appends(['search' => request('search')])->links() }}</div>
        ...
        <div x-show="showModal" style="display:none;" class="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div class="bg-white rounded-3xl p-6 w-full max-w-lg z-10 shadow-2xl relative" x-transition>
                ...
                <form :action="editMode ? '{{ url('achievements') }}/' + currentId : '{{ url('achievements') }}'" method="POST" class="space-y-4 max-h-[70vh] overflow-y-auto">
                    @csrf
                    ...
                    <div>
                        <label class='block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2'>reward</label>
                        <input type='text' name='reward' x-model='form.reward' class='w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500' required>
                    </div>
                    <div>
                        <label class='block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2'>achieved_at</label>
                        <input type='text' name='achieved_at' x-model='form.achieved_at' class='w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500' required>
                    </div>
