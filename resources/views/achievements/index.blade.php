        ...
        <div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
            <table class="w-full text-left text-sm whitespace-nowrap">
                <thead class="bg-slate-50 text-slate-500 text-xs font-semibold uppercase tracking-wider">
                    <tr>
                        <th class='px-6 py-4'>name</th><th class='px-6 py-4'>description</th><th class='px-6 py-4'>reward</th><th class='px-6 py-4'>achieved_at</th>
                ...
                <tbody class="divide-y divide-slate-100 text-slate-700">
                    ...
                    <tr class="hover:bg-slate-50">
                        <td class='px-6 py-4 truncate max-w-xs'>{{ $item->name }}</td><td class='px-6 py-4 truncate max-w-xs'>{{ $item->description }}</td><td class='px-6 py-4 truncate max-w-xs'>{{ $item->reward }}</td><td class='px-6 py-4 truncate max-w-xs'>{{ $item->achieved_at }}</td>
                        <td class="px-6 py-4 text-right">
                            <button @click="showModal = true; editMode = true; currentId = {{ $item->id }}; form = {'title': '{{ addslashes($item->name ?? "") }}', 'description': '{{ addslashes($item->description ?? "") }}', 'reward': '{{ addslashes($item->reward ?? "") }}', 'achieved_at': '{{ addslashes($item->achieved_at ?? "") }}'}" class="text-indigo-600 hover:text-indigo-800 mx-2 text-xs"><i class="fa-solid fa-pen"></i></button>
