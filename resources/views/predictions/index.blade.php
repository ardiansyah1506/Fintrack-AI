<x-app-layout title="AI Predictions - AI Center">
    <x-slot name="header">AI Predictions</x-slot>
    <div x-data="{ showModal: false, editMode: false, currentId: null, form: {'prediction_type': '', 'title': '', 'description': '', 'confidence': '', 'generated_at': '', 'status': ''} }" class="space-y-6 text-slate-800 font-sans relative">
        <div class="flex justify-between items-center bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
            <div>
                <h2 class="font-bold text-xl text-slate-800">Manajemen AI Predictions</h2>
            </div>
            <button @click="showModal = true; editMode = false; form = {'prediction_type': '', 'title': '', 'description': '', 'confidence': '', 'generated_at': '', 'status': ''}" class="bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2.5 rounded-2xl shadow-sm text-sm font-semibold transition">
        ...
        <div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
            <table class="w-full text-left text-sm whitespace-nowrap">
                <thead class="bg-slate-50 text-slate-500 text-xs font-semibold uppercase tracking-wider">
                    <tr>
                        <th class='px-6 py-4'>prediction_type</th><th class='px-6 py-4'>title</th><th class='px-6 py-4'>description</th><th class='px-6 py-4'>confidence</th>
                ...
                <tbody class="divide-y divide-slate-100 text-slate-700">
                    ...
                    <tr class="hover:bg-slate-50">
                        <td class='px-6 py-4 truncate max-w-xs'>{{ $item->prediction_type }}</td><td class='px-6 py-4 truncate max-w-xs'>{{ $item->title }}</td><td class='px-6 py-4 truncate max-w-xs'>{{ $item->description }}</td><td class='px-6 py-4 truncate max-w-xs'>{{ $item->confidence }}</td>
                        <td class="px-6 py-4 text-right">
                            <button @click="showModal = true; editMode = true; currentId = {{ $item->id }}; form = {'prediction_type': '{{ addslashes($item->prediction_type ?? "") }}', 'title': '{{ addslashes($item->title ?? "") }}', 'description': '{{ addslashes($item->description ?? "") }}', 'confidence': '{{ addslashes($item->confidence ?? "") }}', 'generated_at': '{{ addslashes($item->generated_at ?? "") }}', 'status': '{{ addslashes($item->status ?? "") }}'}" class="text-indigo-600 hover:text-indigo-800 mx-2 text-xs"><i class="fa-solid fa-pen"></i></button>
                            <form action="{{ url('predictions', $item->id) }}" method="POST" class="inline">
                                @csrf @method('DELETE')
        ...
        <div class='mt-4'>{{ $predictions->appends(['search' => request('search')])->links() }}</div>
        ...
        <div x-show="showModal" style="display:none;" class="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div class="bg-white rounded-3xl p-6 w-full max-w-lg z-10 shadow-2xl relative" x-transition>
                ...
                <form :action="editMode ? '{{ url('predictions') }}/' + currentId : '{{ url('predictions') }}'" method="POST" class="space-y-4 max-h-[70vh] overflow-y-auto">
                    @csrf
                    <template x-if="editMode"><input type="hidden" name="_method" value="PUT"></template>
                    
                    <div>
                        <label class='block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2'>prediction_type</label>
                        <input type='text' name='prediction_type' x-model='form.prediction_type' class='w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500' required>
                    </div>
                    <div>
                    ...
                    <div>
                        <label class='block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2'>confidence</label>
                        <input type='text' name='confidence' x-model='form.confidence' class='w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500' required>
                    </div>
                    <div>
                        <label class='block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2'>generated_at</label>
                        <input type='text' name='generated_at' x-model='form.generated_at' class='w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500' required>
                    </div>
                    <div>
                        <label class='block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2'>status</label>
                        <input type='text' name='status' x-model='form.status' class='w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500' required>
                    </div>
